    ...
    switch (~~status.mtSt) {
        ...
        case 3: // 3rd period (period number: 3)
            if (parseInt(status.c.remT) < 1)
                printDatchTime = parseInt(status.c.mtT.split(":", 1)) + "'"; // ako je zadnja minuta perioda ne smije dodati 1
            else
        ...
        case 7: // 2nd half (period number: 2)
            if (status.c.stoT) {
                printDatchTime = parseInt(status.c.mtT.split(":", 1)) + "'+" + (parseInt(status.c.stoT.split(":", 1)) + 1);
            }
            else {
                printDatchTime = parseInt(status.c.mtT.split(":", 1)) + 1 + "'"; // minuta utakmice
            }
